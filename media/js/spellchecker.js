(function() {
    ...
    var _handleWindow = function() {
        ...
        for (var i = 0; i < this._spellDat.incorrect.length; i++) {
            if (el.innerHTML == this._spellData.incorrect[i]) {
                for (var s = 0; s < this._spellData.suggestions[this._spellData.incorrect[i]].length; s++) {
                    list = list + '<li title="Replace (' + this._spellData.incorrect[i] + ') with (' + this._spellData.suggestions[this._spellData.incorrect[i]] + ')">' + this._spellData.suggestions[this._spellData.incorrect[i]] + '</li>';
                }
            }
        }
    ...
    myEditor._checkSpelling = function(o) {
        ...
        var that = this;
        $.post("spellcheck", { "tocheck": html },
         function(data){
            that._spellData = data;
        }, "json");

        for (var i = 0; i < this._spellData.length; i++) {
            html = html.replace(new RegExp(this._spellData.incorrect[i], 'g'), '<span class="yui-spellcheck">' + this._spellData.incorrect[i] + '</span>');
        }
